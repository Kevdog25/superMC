...
MCnucl::MCnucl(ParameterReader* paraRdr_in)
{
  ...
  if(shape_of_entropy==3)
  {
    ...
    quark_dist_width = gaussCal->width;
    entropy_gaussian_width = sqrt(quark_dist_width*quark_dist_width+quark_width*quark_width);
    gaussDist = new GaussianDistribution(0, quark_dist_width);
  }
  //For added gluon field texturing
  gluonFieldSwitch = paraRdr->getVal("gluon_field_toggle");
...
{
  int * mapping_table1 = new int[(int)nucl1.size()];
  ...
  if(nucl1.size() == 0){
    ...
    for(int i=0;i<(int)nucl1.size();i++) { // loop over proj. nucleons
      ...
      for(int j=0;j<(int)nucl2.size();j++) { // loop over targ. nucleons
        ...
        if(hit(sqrt(dc))) {
          Ncoll++;
          //Build up the participant array (Kevin)
          buildParticipants(nucl1[i],i,mapping_table1);
          buildParticipants(nucl2[j],i,mapping_table2);
...
void MCnucl::buildParticipants(Particle* nucl,int i,int* mapping_table){
    nucl->setNumberOfCollision();
    // push targ. nucleon j to participant stack (only once though)
    if(nucl->getNumberOfCollision()==1)
    {
      participant.push_back(new Participant(nucl,2));
      if(CCFluctuationModel > 5)
      {
        if(shape_of_entropy == 3)
        {
      ...
      mapping_table[i] = participant.size()-1;
    }
}
...
void MCnucl::setDensity(int iy, int ipt)
{
  ...
  else if(which_mc_model==5) // MC-Glb, entropy centered at wounded nucleons and binary collision centers
  {
      ...
      if (sub_model==1) // "classical" Glb
      {
          ...
          double gluonFieldFlucFactor = 1.0;
          for(unsigned int ipart=0; ipart<participant.size(); ipart++) 
          {
            ...
            for(int ir = x_idx_left; ir < x_idx_right; ir++)
            {
               ...
               for(int jr = y_idx_left; jr < y_idx_right; jr++)
               {
                   double yg = Ymin + jr*dy;
                   double dc = (x-xg)*(x-xg) + (y-yg)*(y-yg);
                   //Relax the cut off for entropy summing if quarks are involved
                   double minDistanceToEntropySource = dc;
                   if(shape_of_entropy == 3)
                   {
                    for(int qIter = 0; qIter < 3; qIter ++)
                    {
                      double d = 0;
                      for(int i = 0; i <3;i++)
                      {
                        d+=(participant[ipart]->getParticle()->ValenceQuarks[qIter][i])*(participant[ipart]->getParticle()->ValenceQuarks[qIter][i]);
                      }
                      if(d < minDistanceToEntropySource)
                        minDistanceToEntropySource = d;
                    }
                   }

                   if(gluonFieldSwitch == 1)
                      gluonFieldFlucFactor = participant[ipart]->getGluonFluc(xg,yg,sqrt(dc),ecm);
                   ...
                   {
                     // skip distant nucleons, speeds things up; one may need to relax
                     if (minDistanceToEntropySource > dc_sq_max_gaussian) continue;
                     double density;
                     if (shape_of_entropy == 3)
                     {
                     ...
                     rhop[ir][jr] += density*gluonFieldFlucFactor;
                   }
               }
            }
          }
